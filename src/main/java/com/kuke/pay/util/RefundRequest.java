...
public class RefundRequest {
    ...
    private void initCert(String path) throws IOException, KeyStoreException, UnrecoverableKeyException, NoSuchAlgorithmException, KeyManagementException {
        //拼接证书的路径
        path = path + Constants.WECHAT_CERTLOCALPATH;
        KeyStore keyStore = KeyStore.getInstance("PKCS12");

        //加载本地的证书进行https加密传输
        FileInputStream instream = new FileInputStream(new File(path));
        try {
            keyStore.load(instream, Constants.WECHAT_MCH_ID.toCharArray());  //加载证书密码，默认为商户ID
        ...
        SSLContext sslcontext = SSLContexts.custom()
                .loadKeyMaterial(keyStore, Constants.WECHAT_MCH_ID.toCharArray())       //加载证书密码，默认为商户ID
                .build();
