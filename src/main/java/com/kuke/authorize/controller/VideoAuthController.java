...
public class VideoAuthController extends BaseController {
	...
	public @ResponseBody ResponseMsg video(HttpServletRequest request) {
		String video_id = request.getParameter("video_id");// 视频ID
		String retStr = "";//返回参数
		...
		if (null != orgToken && !"".equals(orgToken)) {
			...
			if (org.getOrg_status().equals(KuKeAuthConstants.SUCCESS)) {
				//机构登录用户
				OrgChannel  orgChannel =  OrgAuthUtils.getInstance().AuthOrg(org.getOrg_ssoid(), org.getOrg_id(), KuKeAuthConstants.VIDEO);
				//机构播放权限
		...
		if (retStr == ""||retStr.indexOf("message")>-1) {
			if (null != userToken && !"".equals(userToken)) {
				...
				if (user.getUser_status().equals(KuKeAuthConstants.SUCCESS)) {
					...
					// 票据验证通过
					expires_in = userAuthorizeService.playVideoMonthByUserId(user.getUid());
					if (null == expires_in || "".equals(expires_in)) {
						// 时段过期 单次点播
						expires_in = userAuthorizeService.playVideoByUserId(user.getUid(),video_id);
					}
					if (null != expires_in && !"".equals(expires_in)) {
						// 播放权限
						flag = true;
		...
		logger.debug(MessageFormat.format("{0}  验证视频播放权限", retStr));
