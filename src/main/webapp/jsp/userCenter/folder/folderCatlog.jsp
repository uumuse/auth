...
<!-- 				<a href="javascript:void(0)" class="comSite243a1">最近播放</a> -->
			...
			<div class="comSite245">
				<div class="comSite266 comSite266a"></div>
				<div class="comSite266 comSite266a" style="display: none;">
					...
					<div id="catlogdivcontent" class="comSite273">
							<c:if test="${count != 0}">
								...
								<c:forEach  var="item" items="${CatalFolder}" varStatus="status">
										<li class="comSite275 comSite275a">
											<div class="comSite276 pr">
												...
												<span class="comSt53 comSt53c" onclick="toDetail('2','${item.id}');" style="cursor: pointer;"></span>
												<a href="javascript:void(0)" class="comSt252 comSt252b" onclick="toDlPlay('2','${item.id}','play');"></a>
											</div>
											<div class="comSite261 comSite261a pr">
												<p>${item.foldername}（<span>${item.countres}</span>）</p>
												<div class="comSite262 comSite262c">
													<span title="添加到播放列表" class="comSt253 comSt253c" onclick="toDlPlay('2','${item.id}','add');"></span>
										...
										<c:if test="${(status.index + 1) % 4 == 0 && (status.count != fn:length(CatalFolder) )}">
											</ol>
											<ol class="comSite274 comSite274a cf">
										</c:if>
								</c:forEach>
			...
			if(more==true){
				$.ajax({
					...
					success : function(info,textStatus) {
						...
						if (info.flag == true) {
							more = dataInfo.more;
							$("#catlogdivcontent").append(dataInfo.scrollInfo);
							//初始化js
							init();
						}
					},
